...
const SUBJECT = { name: "DeShawn M.", organization: "Fresh Cutz Barbershop", location: "Atlanta, GA" };

const CONTRIBUTOR = { name: "Jordan K." };
...
const DEBATE_POSTS = [
  {
    ...
    name: "DeShawn M.",
    body: "The style in that reference photo required a specific texture and length that this client's hair didn't have. I told him before I started that I could get him close but not exact. He approved it during the cut and only raised an issue after leaving. I work on 20+ clients a week and have never had this complaint before.",
  ...
  },
  {
    ...
    name: "Jordan K.",
    body: "You never told me it wasn't achievable before you started. You took the photo, looked at it, and started cutting. I only said it was \"okay\" because I wasn't sure if it was still being shaped. When I asked about the unevenness you said \"that's how it is.\" That's not communication — that's dismissal.",
  ...
  },
  {
    ...
    role: "subject" as const,
    name: "DeShawn M.",
    body: "I understand you weren't happy with the result. But calling it completely different from the reference isn't accurate. The fade style was achieved — the exact texture wasn't possible with your hair length at the time. That's a hair limitation, not a skill issue.",
...
const SEED_VOTES: VoteRow[] = [
  {
    ...
    jobTitle: "Master Barber, 8 years",
...
const SEED_DEBATE_REACTIONS: Record<string, { agree: number; disagree: number; mine: 1 | -1 | null }> = {
  d2: { agree: 7, disagree: 18, mine: null },
  d3: { agree: 22, disagree: 5, mine: null },
  d4: { agree: 5, disagree: 16, mine: null },
};
...
export default function DemoBarberPage() {
  ...
  const [recordReaction, setRecordReaction] = useState<{ agree: number; disagree: number; mine: 1 | -1 | null }>({ agree: 24, disagree: 7, mine: null });
