...
export default function SupportPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!topic.trim() || !message.trim() || !email.trim() || !firstName.trim()) {
    ...
    try {
      ...
      const { error: insertError } = await supabase.from("support_tickets").insert({
        ...
        status: "open",
        priority: "normal",
        admin_note: !session?.user ? `Name: ${firstName.trim()} | Email: ${email.trim()}` : null,
      });

      if (insertError) throw insertError;
      setSubmitted(true);
      setTopic(""); setEmail(""); setMessage(""); setCategory("general"); setFirstName("");
    } catch (err: any) {
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <button
            onClick={async () => {
              const { data: { session } } = await supabase.auth.getSession();
              if (session?.user) {
                window.location.href = "/dashboard/settings/support";
              } else {
                setShowForm(true);
                setSubmitted(false);
