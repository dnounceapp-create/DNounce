...
import { AlertTriangle, CircleAlert, Pin } from "lucide-react";
...
const DEFAULT_SORT = "Newest Pinned" as const;
...
function RecordMeta({ record }: { record: PinnedRecord }) {
...
export default function PinnedRecordsPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const mapped: PinnedRecord[] = pins.map((p: any) => ({
          pin_id: p.id,
        ...
        console.error("Failed to fetch pinned records:", err);
      } finally {
  ...
  async function unpin(pinId: string) {
    const { data: { session } } = await supabase.auth.getSession();
    if (!session?.user) return;
    await supabase.from("pinned_records").delete().eq("id", pinId);
    setRecords((prev) => prev.filter((r) => r.pin_id !== pinId));
  }
  ...
  const statusToPredicate = (status?: string) => {
    ...
    const map: Record<string, (r: PinnedRecord) => boolean> = {
  ...
  const sortComparator = (key: string): ((a: PinnedRecord, b: PinnedRecord) => number) => {
    switch (key) {
      case "Newest Pinned": return (a, b) => new Date(b.pinned_at).getTime() - new Date(a.pinned_at).getTime();
      case "Oldest Pinned": return (a, b) => new Date(a.pinned_at).getTime() - new Date(b.pinned_at).getTime();
    ...
    <div className="min-h-screen bg-gray-50">
      <main className="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto">
        ...
        <div className="flex flex-col gap-3 sm:gap-4 md:flex-row md:items-center md:justify-between mb-6 sm:mb-8 bg-white/90 backdrop-blur-sm shadow-sm rounded-2xl px-4 sm:px-6 py-3 sm:py-4 border border-gray-100">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <option>Newest Pinned</option>
                <option>Oldest Pinned</option>
        ...
        <div className="bg-white shadow rounded-xl border border-gray-100">
          ...
          {loadingData ? (
            <div className="p-6 text-center text-gray-500">Loading pinned records…</div>
          ) : displayRecords.length === 0 ? (
            <div className="p-8 text-center text-gray-500">
              <Pin className="w-8 h-8 mx-auto mb-2 text-gray-300" />
              <p className="font-medium">No pinned records yet.</p>
              <p className="text-sm mt-1">Pin records from the record detail page to access them here.</p>
            </div>
          ) : (
            paginatedRecords.map((record) => (
              <div key={record.pin_id} className="md:grid md:grid-cols-4 md:items-center gap-3 px-4 sm:px-6 py-4 md:py-5 border-t first:border-t-0 hover:bg-gray-50/50 transition">
                ...
                <div className="mt-3 md:mt-0 text-left md:text-center">
                  <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-[11px] sm:text-xs font-medium ${
                    record.credibility === "Evidence-Based" ? "bg-green-100 text-green-700"
                    : record.credibility === "Opinion-Based" ? "bg-red-100 text-red-700"
                ...
                <div className="mt-3 md:mt-0 flex md:justify-center items-center gap-2">
                  <span className="text-[11px] sm:text-xs text-gray-400">{new Date(record.pinned_at).toLocaleDateString()}</span>
                  <button
                    onClick={() => unpin(record.pin_id)}
                    title="Unpin record"
                    className="inline-flex items-center justify-center rounded-full border border-blue-200 p-1.5 text-blue-600 bg-blue-50 hover:bg-blue-100 transition shrink-0"
                  >
